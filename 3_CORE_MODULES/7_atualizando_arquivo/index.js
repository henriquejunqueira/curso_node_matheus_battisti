...
const server = http.createServer((req, res) => {
  ...
  if (!name) {
    ...
    // Concatena o valor do name, adiciona uma vírgula e faz a quebra de linha
    const nameNewLine = name + ',\r\n';

    // Adiciona o valor do parâmetro 'name' no arquivo 'arquivo.txt' que vem em nameNewLine
    fs.appendFile('arquivo.txt', nameNewLine, function (err, data) {
      // Define o código de status da resposta como 302 (Redirecionamento)
      res.writeHead(302, {
        Location: '/', // Redireciona para a raiz ('/')
      });
